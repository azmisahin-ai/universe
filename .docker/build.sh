...
case "$ARCH" in
    linux_64)
        ...
        nasm -F dwarf -g -f elf64 -o "$PROJECT_FOLDER/build/release/$ARCH/universe.o" -D"$ARCH" "$PROJECT_FOLDER/src/universe/universe.asm"
    ...
    win_64)
        mkdir -p "$PROJECT_FOLDER/build/release/$ARCH"
        nasm -f win64 -o "$PROJECT_FOLDER/build/release/$ARCH/universe.obj" -D"$ARCH" "$PROJECT_FOLDER/src/universe/universe.asm"
    ...
    win_32)
        mkdir -p "$PROJECT_FOLDER/build/release/$ARCH"
        nasm -f win32 -o "$PROJECT_FOLDER/build/release/$ARCH/universe.obj" -D"$ARCH" "$PROJECT_FOLDER/src/universe/universe.asm"
        i686-w64-mingw32-ld -o "$PROJECT_FOLDER/build/release/$ARCH/universe.exe" "$PROJECT_FOLDER/build/release/$ARCH/universe.obj"
